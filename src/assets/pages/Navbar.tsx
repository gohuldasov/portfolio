  ...
  return (
    <nav
      className={`fixed z-50 transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) flex items-center justify-between
        ${isScrolled
          ? 'top-4 left-1/2 -translate-x-1/2 w-fit rounded-full bg-black/80 backdrop-blur-md shadow-2xl py-3 px-8'
          : 'top-0 left-0 w-full bg-transparent py-6 px-30'
        }
      `}
    >
      <div className={`text-lg font-semibold text-white tracking-tight ${isScrolled ? 'hidden md:block mr-22' : ''}`}>
        GD
      </div>
      <div className="flex items-center gap-8 text-xs mr-20 font-medium text-gray-400">
        {navLinks.map((link) => (
          <a
            key={link.name}
            href={link.href}
